...
void main() {
  ...
  group('build()', () {
    test(
      'SharedPreferences가 비어있으면 기본값(알림 OFF, 21:00, alwaysNotify=false)으로 초기화된다',
      () async {
  ...
  group('setAlwaysNotify()', () {
    test('false로 변경하면 state.alwaysNotify가 false가 된다', () async {
      ...
      expect(
        container.read(notificationSettingsProvider).value?.alwaysNotify,
        isFalse,
      );
    });
  ...
  group('setTime()', () {
    ...
    test('state.time이 새 시각으로 변경된다', () async {
      ...
      expect(container.read(notificationSettingsProvider).value?.time, newTime);
    });
